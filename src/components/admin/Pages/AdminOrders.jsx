...
const AdminOrders = () => {
    ...
    const fetchOrders = async (pageNum = 1, append = false, search = '') => {
        try {
            setLoading(true);
            const { data } = await axios.get(`/api/orders?page=${pageNum}&search=${search}`, {
    ...
    const handleUpdateSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`/api/orders/${selectedOrder._id}/status`, updateForm, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            fetchOrders();
                                ...
                                {selectedOrder.orderItems.map((item, index) => (
                                    <div key={index} className="flex gap-4 items-center p-3 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                                        <div className="w-16 h-16 bg-white border rounded-lg flex items-center justify-center overflow-hidden shrink-0">
                                            <img src={item.image.startsWith('http') ? item.image : `/api${item.image}`} className="w-full h-full object-contain" alt="" />
                                        </div>
