...
// Ensure Category model is registered before any populate('category') calls
...
const getProducts = asyncHandler(async (req, res) => {
    ...
    if (usageCategory) {
            // usageCategory can be comma-separated; normalize and match case-insensitively
            ...
            if (cleaned.length > 0) {
                query.usageCategory = { $in: cleaned.map(v => new RegExp(`^${escapeRegExp(v)}$`, 'i')) };
            }
    }
    if (allInOneType) {
