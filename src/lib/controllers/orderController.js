...
const createCloverPayment = asyncHandler(async (req, res) => {
  ...
  try {
      ...
      if (orderId) {
          try {
             // Payment failed, delete the pending order so it doesn't stay in history
      ...
      res.status(400);
      throw new Error(error.response?.data?.message || 'Payment failed. Order was not placed.');
  }
});
...
const deleteOrder = asyncHandler(async (req, res) => {
    ...
    if (order) {
        // Only allow user who placed it or admin to delete
