...
const sendMessage = asyncHandler(async (req, res) => {
    ...
    if (chat) {
        ...
        // Notify Admins only if message is from user
        if (!req.user.isAdmin) {
            const io = req.app.get('io');
            // Assuming we populated user, but we didn't populate it yet.
            // Let's refetch with populate to get user name for notification, or just use generic message
            
            // Or just emit existing info, frontend can maybe fetch details.
            // Better to emit simple notification
            io.emit('new-chat', {
                id: chat._id,
                message: `New message in chat`, // Simplify for now, or fetch user
                path: '/admin/chat',
                time: new Date()
            });
        }
